...
function WorkflowList({ select, submitted }: { select: (id: string) => void; submitted: number }) {
  ...
  const filtered = (data ?? []).filter(workflow => (status === 'ALL' || workflow.status === status) && `${workflow.name} ${workflow.id}`.toLowerCase().includes(query.toLowerCase()));
...
function Dag({ tasks, selected, select }: { tasks: Task[]; selected: string | null; select: (id: string) => void }) {
  ...
  return <div className="dag-scroll" aria-label="Workflow dependency graph"><div className="dag-canvas" style={{ width: layout.width, height: layout.height }}>
